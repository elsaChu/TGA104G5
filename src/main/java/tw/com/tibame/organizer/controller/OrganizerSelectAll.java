...
//		(已解決 只是再JSP打錯字幹 req-> request (OK) )
		...
		if(referer == null) {
			...
			boolean fromSame = referer.equals("http://localhost:8080/JHWorkspace/OrganizerSelectAll");
			System.out.println("fromSame: " + fromSame);
			if(!fromSame) {
				//dont show
				System.out.println("set saA with Value");
		...
		System.out.println("set saA as null");
		req.setAttribute("saAttribute", null);
		res.sendRedirect(req.getContextPath() + "/back-organizer-end/OrganizerSelectAll.jsp");	
