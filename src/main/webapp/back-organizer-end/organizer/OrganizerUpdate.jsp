...
<%-- 		link href="${pageContext.request.contextPath}/css/sample.css" --%>
	...
	<body>
		...
		<div class="login" name="SeachAll">
			...
			<div class="login-top">
				<h1>搜尋</h1>
				<form action="<%=request.getContextPath()%>/OrganizerUpdate" name="OrganizerSelectAllForm"
					method="POST">
							<h1>
...
<!-- 					<input type="text" placeholder="帳號" name="OAccount" /> -->
		...
		<%if(allList == null || (allList.size() == 0)){System.out.println("allList is null, or allList.size() == 0 ");}%>
		<%if (allList != null && (allList.size() > 0)){	%>
		
		<div class="showList">
			<font size="+3">所有廠商：</font>
			<div>
				<table borganizer1="1" width="1000" id="resultTable">
					<tr bgcolor="#999999">
					...
					<%for (int index = 0; index < allList.size(); index++) {OrganizerVO organizer1 = allList.get(index);%>
					<tr>     
						...
						<td width="100"><div align="center">	
							<form action="<%=request.getContextPath()%>/OrganizerUpdateOne" method="post">
								<input type="hidden" name="whichRow" value="<%=organizer1.getOrganizerNumber()%>">
